...
echo
sleep 7
useradd -m -g users -G daemon,disk,wheel,locate,rfkill,dbus,network,video,audio,storage,power,users,input -s /bin/bash maria

echo
echo -e "Nao esqueca de mudar o nome do usuario"
echo
usermod -a -G daemon,disk,wheel,locate,rfkill,dbus,network,video,audio,storage,power,users,input maria
...
echo -e "ExecStart=-/usr/bin/agetty --autologin maria --noclear %I $TERM" >> /etc/systemd/system/getty@tty1.service.d/override.conf
...
echo Nao esqueca de mudar o nome do usuario
echo -e "ExecStart=-/usr/bin/agetty --autologin maria -s %I 115200,38400,9600 vt102" >> /etc/systemd/system/serial-getty@ttyS0.service.d/autologin.conf
